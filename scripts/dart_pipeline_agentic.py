...
- Do NOT write any explanation, summary, or markdown before the JSON.
- Do NOT use **bold**, bullet points, or any natural language.
...
event_type — choose the BEST match:
  "earnings"     : quarterly/annual results, revenue, profit, dividends,
                   share buyback (자기주식취득/처분), stock options, capital changes
  "regulation"   : government policy, export controls, sanctions, compliance
  "macro"        : interest rates, exchange rates, economic indicators
  "geopolitical" : war, trade disputes, political risk
  "product"      : new product launch, R&D, patents, technology
  "personnel"    : CEO/executive change, labor disputes, key person events
  → NEVER use "personnel" for share buyback or stock option filings.

horizon — choose based on when the impact will materialize:
  "intraday" : price impact expected within hours (e.g. earnings surprise)
  "short"    : days to weeks (e.g. regulation announcement)
  "mid"      : weeks to months (e.g. new product launch)
  "long"     : months to years (e.g. strategic investment)
  → Share buyback/disposal filings are typically "short" or "mid", NOT "intraday".
...
# ── 테스트 로그 저장 ─────────────────────────────────────────
def save_test_log(filings: list, scored_signals: list,
                  total_input: int, total_output: int,
                  total_tool_calls: int, consolidated: dict = None,
                  note: str = ""):
    today    = datetime.now().strftime("%Y-%m-%d")
    log_path = Path(f"test_log_{today}.md")
    ...
    entry = f"""
### [{now}] dart_pipeline_agentic.py

| 항목 | 내용 |
|---|---|
| 분석 건수 | {len(filings)}건 → 유효 {len(scored_signals)}건 |
| Tool Use 총 호출 | {total_tool_calls}회 |
| signal_score | {cs.get('signal_score', 'N/A')} |
| opinion | {cs.get('opinion', 'N/A')} |
| confidence | {cs.get('confidence', 'N/A')} |
| trigger | {cs.get('trigger', 'N/A')} |
| 토큰 (입력/출력) | {total_input:,} / {total_output:,} |
| 비용 | ${total_cost_usd:.4f} |
| 특이사항 | {note} |
...
def run(kinds=None, after=None, limit=None):
    ...
    if not scored_signals:
        print("\n유효한 시그널 없음 — 집계 생략")
        save_test_log(filings, scored_signals,
