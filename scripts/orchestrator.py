...
def save_test_log(scenario: str, final: dict,
                  agent_results: dict, note: str = ""):
    ...
    # ── 메타 로그 ──────────────────────────
    meta = f"""
### [{now}] orchestrator.py — {scenario.upper()}

| 항목 | 내용 |
|---|---|
| 시나리오 | {scenario} |
| 결정 | {final.get('decision', 'N/A')} |
| urgency | {final.get('urgency', 'N/A')} |
| confidence | {final.get('confidence', 'N/A')} |
| rebalance_plan | {final.get('rebalance_plan', {})} |
| 호출 에이전트 | {final.get('agents_called', [])} |
| Fast Track | {final.get('fast_track', False)} |
...
"""
    ...
    # ── 판단 결과 상세 ──────────────────────
    detail = f"""
### [{now}] orchestrator.py — 판단 결과 ({scenario.upper()})
...
"""
    for agent_name, result in agent_results.items():
        if result.get("status") != "ok":
            detail += f"- {agent_name}: [{result.get('status')}] {result.get('reason','')}\n"
            continue
        data = result.get("data", {})
        detail += f"""- {agent_name}
  signal_score={data.get('signal_score', 'N/A')}  opinion={data.get('opinion', 'N/A')}  confidence={data.get('confidence', 'N/A')}
  reason: {result.get('reason', '')}
"""
    detail += "\n"
...
Do NOT write any explanation, summary, or markdown before or after the JSON."""
...
def run_mock_orchestrator(portfolio: dict, scenario: str) -> dict:
    ...
    # ── decisions DB 저장 ─────────────────────────────
    save_decision(final, agent_results, scenario, constraint_msg)
    save_test_log(scenario, final, agent_results,    # ← 추가
                  note=constraint_msg if constraint_msg else "")
...
Call the necessary agents to gather information, then provide your final decision as JSON."""
    ...
    for _ in range(MAX_ROUNDS):
        ...
        if resp.stop_reason == "tool_use":
            ...
            save_test_log(scenario, final, agent_results)
            return final
