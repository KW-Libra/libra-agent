...
# 8. 로그 저장
# ─────────────────────────────────────────
def save_test_log(articles: list, result: dict,
                  cost: dict, note: str = ""):
    today    = datetime.now().strftime("%Y-%m-%d")
    now      = datetime.now().strftime("%Y-%m-%d %H:%M")
    log_path    = Path(f"test_log_{today}.md")
    result_path = Path(f"test_result_{today}.md")

    cs             = result.get("consolidated_signal", {}) if result else {}
    total_cost_usd = cost.get("total_cost_usd", 0) if cost else 0
...
| 항목 | 내용 |
|---|---|
| 수집 기사 수 | {len(articles)}건 |
| Tool Use 총 호출 | {cost.get('tool_calls', 0) if cost else 0}회 |
| signal_score | {cs.get('signal_score', 'N/A')} |
| opinion | {cs.get('urgency', 'N/A')} |
| confidence | {cs.get('confidence', 'N/A')} |
| trigger | {cs.get('trigger', 'N/A')} |
| 토큰 (입력/출력) | {cost.get('input_tokens', 0):,} / {cost.get('output_tokens', 0):,} |
| 비용 | ${total_cost_usd:.4f} |
| 특이사항 | {note if note else '없음'} |
...
"""
    ...
    if result:
        ...
        relevant = [a for a in articles_analyzed if a.get("relevant")]

        detail = f"""
### [{now}] news_agent.py — 판단 결과

**consolidated_signal**
- signal_score : {cs.get('signal_score', 'N/A')}
- trigger      : {cs.get('trigger', 'N/A')}
- urgency      : {cs.get('urgency', 'N/A')}
- risk_level   : {cs.get('risk_level', 'N/A')}
- tickers      : {cs.get('tickers', [])}
- reason       : {cs.get('reason', 'N/A')}

**관련 기사 ({len(relevant)}건)**
"""
        for a in relevant:
            detail += f"""
- [{a.get('event_type','?')}] {a.get('title','')[:60]}
  sentiment={a.get('sentiment',0):+.2f} impact={a.get('impact',0)} confidence={a.get('confidence',0):.2f}
  signal_score={a.get('signal_score',0):+.4f}
  reason: {a.get('reason','')}
"""
        detail += "\n"
    ...
    print(f"  → 메타 로그: {log_path}")
    print(f"  → 결과 로그: {result_path}")
# ─────────────────────────────────────────
# 9. 메인 실행 사이클 (단독 실행용)
# ─────────────────────────────────────────
...
def run_cycle():
    ...
    if not articles:
        print("  수집된 기사 없음. 종료.")
        save_test_log([], None, {}, note="수집된 기사 없음")
        return
    ...
    if api_key:
        ...
        try:
            ...
            save_test_log(articles, None, {}, note=f"Claude 호출 실패: {e}")
            return
    else:
    ...
    save_to_db(result, articles, run_at)
    save_test_log(articles, result, cost)
    return result
