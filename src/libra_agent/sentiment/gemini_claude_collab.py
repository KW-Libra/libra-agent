...
async def _call_gemini(headlines_block: str, portfolio_summary: str) -> Optional[dict]:
...
def _merge(gemini_out: dict, claude_out: Optional[dict]) -> dict:
    """두 모델 결과를 보수적으로 합친다."""
    ...
    if g_vote == c_vote:
        ...
        final_vote = (
            g_vote if _VOTE_RANK.get(g_vote, 1) <= _VOTE_RANK.get(c_vote, 1) else c_vote
        )
...
async def run_collab(
    ...
    headlines_block = "\n".join(
        f"- [{s.label.upper()} {round(s.score * 100)}%] {s.headline}"
        for s in filtered
    )
    ...
    model_used = (
        "finbert+gemini-flash+claude"
        if not merged["review_skipped"]
        else "finbert+gemini-flash"
    )

    return NewsAnalysisResult(
        portfolio_sentiment_score=merged["score"] if merged["score"] is not None else fallback_score,
        vote=merged["vote"],
