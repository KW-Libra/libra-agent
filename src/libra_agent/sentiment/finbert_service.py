...
class FinBERTService:
    ...
    def _score_sync(self, headlines: list[str]) -> list[HeadlineSentiment]:
        ...
        if not self._load() or self._pipeline is None:
            # 폴백: 모두 neutral
            return [HeadlineSentiment(headline=h, label="neutral", score=0.5) for h in headlines]
        ...
        for i in range(0, len(headlines), BATCH_SIZE):
            ...
            try:
                ...
                for headline, label_scores in zip(batch, raw_outputs, strict=False):
                ...
                results.extend(
                    [HeadlineSentiment(headline=h, label="neutral", score=0.5) for h in batch]
                )
