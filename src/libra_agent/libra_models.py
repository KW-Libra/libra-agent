...
def _normalize_verdict(value: Any, default: AgentVerdict = AgentVerdict.PARTIAL_ANSWER) -> AgentVerdict:
    try:
...
def _normalize_phase(value: Any, default: DecisionPhase = DecisionPhase.INFORMATION_GATHERING) -> DecisionPhase:
    try:
...
class AgentResponse:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> AgentResponse:
        references = [Reference.from_dict(item) for item in _as_list(payload.get("references")) if isinstance(item, Mapping)]
        tools_called = [ToolCall.from_dict(item) for item in _as_list(payload.get("tools_called")) if isinstance(item, Mapping)]
        return cls(
            agent_id=_as_str(payload.get("agent_id")),
            opinion_id=_as_str(payload.get("opinion_id")),
            ...
            focus_tickers=[_as_str(item) for item in _as_list(payload.get("focus_tickers")) if _as_str(item)],
        )
...
class PortfolioSnapshot:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> PortfolioSnapshot:
        holdings = tuple(PortfolioHolding.from_dict(item) for item in _as_list(payload.get("holdings")) if isinstance(item, Mapping))
        return cls(
            generated_at=coerce_datetime(_as_str(payload.get("generated_at")) or None),
            holdings=holdings,
            total_value_krw=_as_float(payload.get("total_value_krw")) if payload.get("total_value_krw") is not None else None,
            cash_weight=_clamp(_as_float(payload.get("cash_weight"), 0.0), 0.0, 1.0),
            user_preferences=tuple(_as_str(item) for item in _as_list(payload.get("user_preferences")) if _as_str(item)),
        )
...
class KnowledgeDocument:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> KnowledgeDocument:
        return cls(
            ...
            relevance_score=_as_float(payload.get("relevance_score")) if payload.get("relevance_score") is not None else None,
            event_type=_as_str(payload.get("event_type")) or None,
            event_type_score=_as_float(payload.get("event_type_score")) if payload.get("event_type_score") is not None else None,
            entities=tuple(
                KnowledgeEntity.from_dict(item)
                for item in _as_list(payload.get("entities"))
                if isinstance(item, Mapping)
            ),
            matched_holdings=tuple(_as_str(item) for item in _as_list(payload.get("matched_holdings")) if _as_str(item)),
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "relevance_score": round(float(self.relevance_score), 4) if self.relevance_score is not None else None,
            "event_type": self.event_type,
            "event_type_score": round(float(self.event_type_score), 4) if self.event_type_score is not None else None,
...
class JudgeDecision:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> JudgeDecision:
        ...
        return cls(
            ...
            confidence=_clamp(_as_float(payload.get("confidence"), 0.0), 0.0, 1.0),
            urgency=_normalize_urgency(payload.get("urgency")),
            called_agents=[_as_str(item) for item in _as_list(payload.get("called_agents")) if _as_str(item)],
            skipped_agents=[_as_str(item) for item in _as_list(payload.get("skipped_agents")) if _as_str(item)],
            skip_rationale={_as_str(key): _as_str(value) for key, value in _as_dict(payload.get("skip_rationale")).items()},
            ...
            user_notification=UserNotification.from_dict(notification_payload) if isinstance(notification_payload, Mapping) else None,
            ...
            trigger_event=TriggerEvent.from_dict(trigger_event_payload) if isinstance(trigger_event_payload, Mapping) else None,
            deadline_at=_as_str(payload.get("deadline_at")) or None,
            elapsed_seconds=_as_float(payload.get("elapsed_seconds")) if payload.get("elapsed_seconds") is not None else None,
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "user_notification": self.user_notification.to_dict() if self.user_notification else None,
            ...
            "elapsed_seconds": round(float(self.elapsed_seconds), 3) if self.elapsed_seconds is not None else None,
