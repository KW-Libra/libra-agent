...
class SentimentAgent(BaseAgent):
    ...
    async def _try_phase2(self, ctx: PortfolioContext) -> AgentVerdict | None:
        try:
            ...
            portfolio_summary = json.dumps({
                "top_holdings": [
                    {"symbol": h["symbol"], "weight": h.get("weight", 0), "sector": h.get("sector", "")}
            ...
            signals = [
                {"label": "감성 점수",    "value": str(round(score, 2)), "range": "[-1.0 공포~+1.0 탐욕]"},
    ...
    async def _run_phase1(self, ctx: PortfolioContext) -> AgentVerdict:
        ...
        portfolio_summary = json.dumps({
            "top_holdings": [
                {"symbol": h["symbol"], "weight": h.get("weight", 0)}
                for h in top
            ],
        ...
        try:
            ...
            if "reject" in lower: vote = "reject"
            elif "approve" in lower: vote = "approve"

        if sentiment_score < self.FEAR_THRESHOLD: vote = "abstain"
        elif sentiment_score > self.GREED_THRESHOLD: vote = "approve"
