...
class PortfolioOptimizer:
    ...
    def optimize(
        ...
        if method == "equal_weight" or returns_matrix is None:
            return self._equal_weight(symbols, note="히스토리 데이터 없음" if returns_matrix is None else "")
    ...
    def _risk_parity(
        self, symbols: list[str], cov: np.ndarray
    ) -> OptimizationResult:
        """
    ...
    def compute_fifo_cost_basis(
        self, buy_history: list[dict[str, Any]]
    ) -> list[TaxLot]:
        """
