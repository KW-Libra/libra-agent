...
class ComplianceAgent(BaseAgent):
    ...
    async def deliberate(self, ctx: PortfolioContext) -> AgentVerdict:
        ...
        approval_mode = str(preferences.get("approval") or preferences.get("approval_mode") or "manual")
        ...
        for trade in ctx.proposed_trades:
            ...
            holding = next((item for item in ctx.holdings if item.get("symbol") == sym), {"symbol": sym})
            exclusion = _matches_exclusion(holding, exclusions)
            if exclusion:
