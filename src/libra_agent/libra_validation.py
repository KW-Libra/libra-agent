...
def _sanitize_disclosure_evidence(value: Any, alias_map: Mapping[str, str]) -> dict[str, Any]:
    ...
    for item in _as_list(payload.get("items")):
        ...
        headline = _clean_text(
            item.get("headline") or item.get("title") or item.get("disclosure_type") or item.get("type"),
            limit=180,
        )
        ...
        timestamp = _datetime_string(item.get("timestamp") or item.get("published_at") or item.get("date"))
    ...
    for item in _as_list(payload.get("upcoming_disclosures")):
        ...
        date = _datetime_string(item.get("date") or item.get("timestamp") or item.get("scheduled_at"))
...
def _sanitize_report_evidence(value: Any, alias_map: Mapping[str, str]) -> dict[str, Any]:
    ...
    for item in _as_list(payload.get("items")):
        ...
        key_thesis = _clean_text(item.get("key_thesis") or item.get("summary") or item.get("headline"), limit=320)
...
def _sanitize_news_evidence(value: Any, alias_map: Mapping[str, str]) -> dict[str, Any]:
    ...
    if isinstance(company_findings_raw, Mapping):
        for raw_key, raw_value in company_findings_raw.items():
            ...
            canonical_key = "portfolio" if cleaned_key.casefold() == "portfolio" else _canonical_ticker(cleaned_key, alias_map)
            ...
            company_findings[canonical_key] = {
                ...
                "sector_comparison": _clean_text(raw_value.get("sector_comparison"), limit=180) or None,
            }
    ...
    source_reliability = _clean_text(payload.get("source_reliability"), default="medium", limit=20).lower()
...
def _sanitize_profit_evidence(value: Any, alias_map: Mapping[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        "plan_simulation": {
            ...
            "recommendation_text": _clean_text(plan_simulation.get("recommendation_text"), limit=240) or None,
        },
    }
...
def _sanitize_cost_evidence(value: Any, alias_map: Mapping[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        "trade_cost": {
            ...
            "estimated_slippage_bp": round(max(0.0, _as_float(trade_cost.get("estimated_slippage_bp"))), 4),
...
def sanitize_agent_response_payload(
    ...
    *,
    ...
    evidence = sanitize_agent_evidence(agent_id=agent_id, evidence=raw.get("evidence"), portfolio=portfolio)
    alias_map = _build_ticker_alias_map(portfolio)
    ...
    if not focus_tickers:
        if agent_id == "disclosure":
            ...
            focus_tickers = [
                ticker for ticker in evidence.get("company_findings", {}).keys()
                if ticker != "portfolio"
            ]
        else:
            plan = _as_mapping(evidence.get("plan_simulation") or evidence.get("trade_cost")).get("rebalance_plan")
            focus_tickers = _canonical_ticker_list(list(_as_mapping(plan).keys()), alias_map)

    response.agent_id = agent_id
    response.opinion_id = _clean_text(raw.get("opinion_id"), default=opinion_id, limit=120) or opinion_id
    response.turn_number = turn_number
    response.query_understood = _clean_text(raw.get("query_understood"), default=query, limit=240) or query
...
def sanitize_judge_payload(
    ...
    *,
    ...
    summary = _clean_text(raw.get("summary"), default=_default_summary_for_decision(decision), limit=320)
    ...
    user_notification = {
        ...
        "kind": _clean_text(notification.get("kind"), default="final_decision", limit=40) or "final_decision",
