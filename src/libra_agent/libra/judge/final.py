...
from ..schemas.decision import ConsensusBranch, ConsensusScore, DecisionBranch, DecisionType, FinalDecision, Trade, UserOption
...
def determine_branch(
    ...
    if any(score.branch == ConsensusBranch.WEAK_CONSENSUS for score in consensus_per_subject.values()):
        return DecisionType.REBALANCE, DecisionBranch.WEAK_CONSENSUS_CONSERVATIVE
    if consensus_per_subject and all(score.branch == ConsensusBranch.STRONG_HOLD for score in consensus_per_subject.values()):
...
def compute_trade_consensus(subject: str, votes: list[Vote], *, conservative: bool = False) -> Trade:
    eligible = [vote for vote in votes if not vote.informational and vote.direction != Direction.HOLD]
    if not eligible:
        return Trade(subject=subject, delta_pct=0.0, rationale="실행 가능한 방향성 투표가 없습니다.")
...
def compute_tentative_trades(
    ...
    for subject, score in consensus_per_subject.items():
        ...
        if score.branch == ConsensusBranch.STRONG_CONSENSUS:
            ...
            trade = compute_trade_consensus(subject, votes_by_subject.get(subject, []), conservative=True)
        else:
            continue
...
def render_rule_based_final_decision(
    *,
    ...
    if branch == DecisionBranch.COMPLIANCE_VETO:
        ...
        options = [
            UserOption("거래 취소", ["Compliance"], "위반 거래를 실행하지 않고 현재 포트폴리오 유지"),
            UserOption("IPS 한도 일시 완화", ["Final Judge"], "이번 거래 한정으로 사용자 명시 동의 후 재검토"),
            UserOption("다른 자산으로 대체", ["Risk", "Profit"], "위반하지 않는 대체 리밸런싱 경로 탐색"),
        ]
