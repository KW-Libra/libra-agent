...
class PortfolioDefinition:
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> PortfolioDefinition:
        ...
        risk_profile = str(payload.get("risk_profile") or profile.get("risk_profile") or "위험중립형")
        default_threshold = RISK_PROFILE_THRESHOLDS.get(risk_profile.casefold(), 0.05)
        try:
            threshold = float(payload.get("drift_threshold") or profile.get("drift_threshold") or default_threshold)
        except (TypeError, ValueError) as exc:
        ...
        targets = tuple(TargetWeight.from_dict(item) for item in raw_targets if isinstance(item, Mapping))
        if not targets:
            raise ValueError("portfolio_definition.target_weights must contain at least one valid item.")

        total_weight = sum(item.weight for item in targets)
        if not (0.999 <= total_weight <= 1.001):
            raise ValueError(f"portfolio_definition target weights must sum to 1.0. current={total_weight:.4f}")
        ...
        return cls(
            ...
            threshold_overridden=bool(payload.get("threshold_overridden") or profile.get("threshold_overridden")),
...
class StockDrift:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "drift_rel": self.drift_rel if math.isinf(self.drift_rel) else round(float(self.drift_rel), 6),
...
class DriftReport:
    ...
    computed_at: str = field(default_factory=lambda: datetime.now().astimezone().isoformat(timespec="seconds"))
...
def compute_drift(definition: PortfolioDefinition, portfolio: PortfolioSnapshot) -> DriftReport:
    ...
    for ticker in all_tickers:
        ...
        stock_drifts.append(
            StockDrift(
                ticker=ticker,
                company_name=(target.company_name if target else holding.company_name if holding else ticker),
    ...
    rms = math.sqrt(sum(item.drift_abs**2 for item in stock_drifts) / len(stock_drifts)) if stock_drifts else 0.0
...
def compact_drift_context(report: DriftReport | Mapping[str, Any] | None) -> dict[str, Any] | None:
    ...
    return {
        ...
        "stock_drifts": [
            ...
            if isinstance(item, Mapping) and str(item.get("severity") or "") in {"MODERATE", "SEVERE"}
        ][:8],
    }
