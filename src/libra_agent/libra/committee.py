...
DISPLAY_TO_AGENT_ID.update({_display: _agent for _agent, _display in {
    "disclosure": "Disclosure",
    "news": "News",
    "report": "Report",
    "profit": "Profit",
        ...
        *,
        ...
        if not mediator_decision.skip_round_2 and mediator_decision.targets_to_recall:
            ...
            _annotate_round2_opinions(
                ...
                exposed_signals=_exposed_signals(round1, exclude_agents={op.agent for op in round2}),
            )
    ...
    *,
    ...
    if final_decision.user_question is not None:
        if not isinstance(question, str) or not question.strip():
            raise ChatClientError("Final Judge must return user_question for USER_DECISION_REQUIRED.")
        ...
        for item in raw_options:
            ...
            user_options.append(
                UserOption(
                    ...
                    supporting_agents=[str(agent) for agent in item.get("supporting_agents", []) if str(agent).strip()]
...
def _deterministic_mediator_rationale(targets: list[str], compliance_before: ComplianceCheck) -> str:
    if not targets:
...
def _exposed_signals(opinions: list[AgentOpinion], *, exclude_agents: set[str] | None = None) -> list[str]:
