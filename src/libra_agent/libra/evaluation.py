...
def evaluate_decision_outcome(
    *,
    ...
    direction_accuracy = _direction_accuracy(signal_score=signal_score, realized_return_pct=realized_return_pct)
    magnitude_error = round(abs(abs(signal_score) * 10.0 - abs(realized_return_pct)), 4)
    cost_efficiency = round(abs(realized_return_pct) - max(cost_pct, 0.0), 4) if rebalance_plan else None
    has_actionable_recommendation = normalized_decision in {"REBALANCE", "USER_DECISION_REQUIRED"} or bool(rebalance_plan)
...
def _direction_accuracy(*, signal_score: float, realized_return_pct: float) -> bool | None:
    if signal_score == 0:
        return None
    return (signal_score > 0 and realized_return_pct > 0) or (signal_score < 0 and realized_return_pct < 0)
