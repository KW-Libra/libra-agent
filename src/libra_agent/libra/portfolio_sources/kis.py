...
def _config_from_args(args: Any) -> KISCredentialConfig:
    ...
    if len(normalized_account) != 8:
        raise KISPortfolioBootstrapError("KIS account number must contain the first 8 digits of the account.")
    if len(normalized_product) != 2:
        raise KISPortfolioBootstrapError("KIS product code must contain the final 2 digits of the account.")
...
class KISDomesticPortfolioClient:
        ...
        for _ in range(10):
            ...
            if not summary_rows:
                if isinstance(page_summary, list):
                    summary_rows = [dict(item) for item in page_summary if isinstance(item, Mapping)]
    ...
    def _to_snapshot(
        ...
        stock_value_sum = _as_float(summary.get("evlu_amt_smtl_amt")) or sum(_as_float(item.get("evlu_amt")) for item in rows)
    ...
    def _raise_for_http(response: httpx.Response, action: str) -> None:
        try:
            ...
            raise KISPortfolioBootstrapError(f"KIS {action} failed with HTTP {response.status_code}.") from exc
