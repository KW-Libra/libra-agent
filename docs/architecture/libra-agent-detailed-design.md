...
>
> **범위**: Judge 오케스트레이터 + 6개 하위 에이전트 (Disclosure / News / Report / Profit / Cost / Evaluation). Portfolio Service(에이전트 아님)는 별도.
>
...
| **Evaluation** | 저장된 판단 결과의 사후 평가 / 실현 수익률·비용·사용자 피드백 비교 / USER_CORRECT·USER_WRONG·ACCURATE·FALSE_POSITIVE 등 평가 라벨 산출 / 판단 품질 기록 | Judge 실행 중 정보 수집 / 신규 매매 판단 / 리밸런싱 계획 수립 / 실시간 주문 실행 |
...
- feedback_checkpoint 예약. 단, 예약 시점에 실제 채점은 Evaluation Agent가 수행
...
| **Judge vs Evaluation** | Judge는 당시 정보로 결정을 내리고 후속 평가 시점을 예약한다. Evaluation은 시간이 지난 뒤 실현 결과로 그 판단이 맞았는지 채점한다. |
...
Evaluation Agent는 Judge-time `AgentResponse`를 반환하지 않는다. 사후 평가 API의 별도 계약을 사용한다.

```python
class EvaluationResult(BaseModel):
    agent_id: Literal["evaluation"]
    horizon: str
    verdict: Literal[
        "USER_WRONG",
        "USER_CORRECT",
    ...
    direction_accuracy: Optional[bool]
...
- Evaluation Agent의 사후 평가 API 분리
...
- [x] Evaluation Agent 사후 평가 계약 분리
...
**2026-04-27** — Evaluation Agent 명시
- Judge-time 정보 수집 에이전트와 사후 평가 에이전트를 분리.
- Evaluation Agent는 `feedback_checkpoint` 이후 저장된 판단, 실현 수익률, 비용, 사용자 피드백을 비교해 판단 품질을 기록한다.
- `/v1/evaluations` API와 backend `evaluations` 저장 테이블을 기준 계약으로 둔다.
